...
public class QuizTest extends JFrame implements  ActionListener{
	...
	JLabel label;
	JRadioButton radioButton[] = new JRadioButton[5];//width of buttons
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("Result")) {
			if (check())
				count = count + 1;//If the answer is correct then add one point
			current++;
	...
	// Checking if the user has selected the right answer or not.
	boolean check() {
		if (current == 0)
